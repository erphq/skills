---
name: realtime
description: Use this skill when an agent needs to add realtime updates via websockets on build.host and return WS server, client hook, room-scoped pub/sub.
version: 0.1.0
agents:
  - build-host
  - codex
...
Add a product feature or integration, then make it deployable. This specific recipe handles the user intent `add realtime updates via websockets` and should end with: WS server, client hook, room-scoped pub/sub.
...
- The user asks for: `add realtime updates via websockets`.
...
- `add realtime updates via websockets`
- `can you add realtime updates via websockets and give me the live URL?`
