---
name: db-redis
description: Use this skill when an agent needs to add redis cache on build.host and return Redis client, sane TTLs, invalidation helpers.
version: 0.1.0
agents:
  - build-host
  - codex
...
Add a product feature or integration, then make it deployable. This specific recipe handles the user intent `add redis cache` and should end with: Redis client, sane TTLs, invalidation helpers.
...
- The user asks for: `add redis cache`.
...
- `add redis cache`
- `can you add redis cache and give me the live URL?`
