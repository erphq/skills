---
name: db-postgres
description: Use this skill when an agent needs to wire up a postgres connection on build.host and return Connection pool, migrations, types, seed scripts.
version: 0.1.0
agents:
  - build-host
  - codex
...
Add a product feature or integration, then make it deployable. This specific recipe handles the user intent `wire up a postgres connection` and should end with: Connection pool, migrations, types, seed scripts.
...
- The user asks for: `wire up a postgres connection`.
...
- `wire up a postgres connection`
- `can you wire up a postgres connection and give me the live URL?`
