---
name: crud-from-schema
description: Use this skill when an agent needs to CRUD UI from this JSON schema on build.host and return Forms, validation, optimistic mutations, RLS-aware.
version: 0.1.0
agents:
  - build-host
  - codex
...
Add a product feature or integration, then make it deployable. This specific recipe handles the user intent `CRUD UI from this JSON schema` and should end with: Forms, validation, optimistic mutations, RLS-aware.
...
- The user asks for: `CRUD UI from this JSON schema`.
...
- `CRUD UI from this JSON schema`
- `can you CRUD UI from this JSON schema and give me the live URL?`
