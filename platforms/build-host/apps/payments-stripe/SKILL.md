...
description: Use this skill when an agent needs to add stripe checkout on build.host and return Checkout session, webhook handler, customer portal.
version: 0.1.0
agents:
  - build-host
  - codex
...
Add a product feature or integration, then make it deployable. This specific recipe handles the user intent `add stripe checkout` and should end with: Checkout session, webhook handler, customer portal.
...
- The user asks for: `add stripe checkout`.
...
- `add stripe checkout`
- `can you add stripe checkout and give me the live URL?`
