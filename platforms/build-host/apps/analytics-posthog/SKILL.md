...
description: Use this skill when an agent needs to add posthog event tracking on build.host and return PostHog client, identified events, feature flags.
version: 0.1.0
agents:
  - build-host
  - codex
...
Add a product feature or integration, then make it deployable. This specific recipe handles the user intent `add posthog event tracking` and should end with: PostHog client, identified events, feature flags.
...
- The user asks for: `add posthog event tracking`.
...
- `can you add posthog event tracking and give me the live URL?`
