---
name: email-resend
description: Use this skill when an agent needs to send transactional email via resend on build.host and return Resend client + templates + dev-mode preview.
version: 0.1.0
agents:
  - build-host
  - codex
...
Add a product feature or integration, then make it deployable. This specific recipe handles the user intent `send transactional email via resend` and should end with: Resend client + templates + dev-mode preview.
...
- The user asks for: `send transactional email via resend`.
...
- `can you send transactional email via resend and give me the live URL?`
