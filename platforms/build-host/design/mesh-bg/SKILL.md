---
name: mesh-bg
description: Use this skill when an agent needs to animated gradient mesh background on build.host and return Drifting radial-gradient mesh, GPU-light, 22s loop.
version: 0.1.0
agents:
  - build-host
  - codex
...
Apply a visual system to an existing app or new surface. This specific recipe handles the user intent `animated gradient mesh background` and should end with: Drifting radial-gradient mesh, GPU-light, 22s loop.
...
- The user asks for: `animated gradient mesh background`.
...
- `can you animated gradient mesh background and give me the live URL?`
