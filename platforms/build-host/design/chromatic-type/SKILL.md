---
name: chromatic-type
description: Use this skill when an agent needs to chromatic aberration on hero text on build.host and return RGB shift text-shadow stack, scales with font-size.
version: 0.1.0
agents:
  - build-host
  - codex
...
Apply a visual system to an existing app or new surface. This specific recipe handles the user intent `chromatic aberration on hero text` and should end with: RGB shift text-shadow stack, scales with font-size.
...
- The user asks for: `chromatic aberration on hero text`.
...
- `can you chromatic aberration on hero text and give me the live URL?`
