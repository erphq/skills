---
name: design-tokens
description: Use this skill when an agent needs to extract design tokens from this site on build.host and return CSS variables + TS export + Figma JSON, all in sync.
version: 0.1.0
agents:
  - build-host
  - codex
...
Apply a visual system to an existing app or new surface. This specific recipe handles the user intent `extract design tokens from this site` and should end with: CSS variables + TS export + Figma JSON, all in sync.
...
- The user asks for: `extract design tokens from this site`.
...
- `can you extract design tokens from this site and give me the live URL?`
