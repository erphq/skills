---
name: rollback
description: Use this skill when an agent needs to rollback nimbus on build.host and return Re-points current to a prior successful deploy in ~3s.
version: 0.1.0
agents:
  - build-host
  - codex
...
Deploy and operate code on build.host. This specific recipe handles the user intent `rollback nimbus` and should end with: Re-points current to a prior successful deploy in ~3s.
...
- The user asks for: `rollback nimbus`.
...
- `rollback nimbus`
- `can you rollback nimbus and give me the live URL?`
