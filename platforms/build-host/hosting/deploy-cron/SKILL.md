---
name: deploy-cron
description: Use this skill when an agent needs to run this script every 5 minutes on build.host and return Scheduled job (cron syntax) with last-run logs and alerts.
version: 0.1.0
agents:
  - build-host
  - codex
...
Deploy and operate code on build.host. This specific recipe handles the user intent `run this script every 5 minutes` and should end with: Scheduled job (cron syntax) with last-run logs and alerts.
...
- The user asks for: `run this script every 5 minutes`.
...
- `can you run this script every 5 minutes and give me the live URL?`
