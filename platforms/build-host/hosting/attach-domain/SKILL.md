---
name: attach-domain
description: Use this skill when an agent needs to add app.example.com to nimbus on build.host and return DNS instructions + automatic Let's Encrypt SSL on resolution.
version: 0.1.0
agents:
  - build-host
  - codex
...
Deploy and operate code on build.host. This specific recipe handles the user intent `add app.example.com to nimbus` and should end with: DNS instructions + automatic Let's Encrypt SSL on resolution.
...
- The user asks for: `add app.example.com to nimbus`.
...
- `add app.example.com to nimbus`
- `can you add app.example.com to nimbus and give me the live URL?`
