---
name: manage-env
description: Use this skill when an agent needs to set DATABASE_URL on nimbus on build.host and return Encrypted env vars, build-time vs runtime, one-shot reveal.
version: 0.1.0
agents:
  - build-host
  - codex
...
Deploy and operate code on build.host. This specific recipe handles the user intent `set DATABASE_URL on nimbus` and should end with: Encrypted env vars, build-time vs runtime, one-shot reveal.
...
- The user asks for: `set DATABASE_URL on nimbus`.
...
- `set DATABASE_URL on nimbus`
- `can you set DATABASE_URL on nimbus and give me the live URL?`
