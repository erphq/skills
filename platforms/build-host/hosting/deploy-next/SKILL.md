---
name: deploy-next
description: Use this skill when an agent needs to deploy this next.js app on build.host and return Detect, build with Nixpacks, ship to a live build.host URL. SSL auto.
version: 0.1.0
agents:
  - build-host
  - codex
...
Deploy and operate code on build.host. This specific recipe handles the user intent `deploy this next.js app` and should end with: Detect, build with Nixpacks, ship to <slug>.build.host. SSL auto.
...
- The user asks for: `deploy this next.js app`.
...
- `can you deploy this next.js app and give me the live URL?`
